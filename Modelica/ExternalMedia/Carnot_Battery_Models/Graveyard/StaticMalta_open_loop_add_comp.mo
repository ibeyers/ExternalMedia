...
model StaticMalta_open_loop_add_comp
  //--------------------------IMPORTS-----------------------------//
  ...
  */
  //-------------HEX 2 LOOP//
  //hot-side
  SI.SpecificHeatCapacity cp_hot_HEX2_charge_loop;
  Real C_hot_HEX2_charge_loop "Heat capacity rate of hot side";
  //cold-side
  Real C_cold_HEX2_charge_loop "Heat capacity rate of cold side ";
  SI.SpecificHeatCapacity cp_cold_HEX2_charge_loop;
  //variables for effectiveness
  Real C_min_HEX2_charge_loop;
  Real C_max_HEX2_charge_loop;
  Real C_r_HEX2_charge_loop;
  Real NTU_HEX2_charge_loop "Number of transfer units of recuperation HEX";
  Real eff_HEX2_charge_loop(start = 0.96) "effectiveness of hot HEX";
  //heat transferred
  SI.HeatFlowRate Q_dot_HEX2_charge_loop(displayUnit = "MW");
  SI.HeatFlowRate Q_dot_max_HEX2_charge_loop(displayUnit = "MW");
  //energy balance and outlet states // only interested in cold side
  SI.SpecificEnthalpy h_cold_out_HEX2_charge_loop;
  WorkingFluid.ThermodynamicState outlet_coldside_HEX2_charge_loop(p(start = 97889), T(start = 540), phase(start = 1)) "Medium properties";
  //-------------HEX 3//
  //design
  ...
  parameter SI.Temperature T_amb = from_degC(19);
  ...
  parameter SI.Temperature T_4_nom_charge = from_degC(267.533) "state 4 temperature";
  parameter SI.Pressure p_4_nom_charge = p_fix_charge "state 4 pressure";
  //actual
  ...
  */
  //-------------COMPRESSOR CHARGE LOOP//
  /*
        Real c1_charge_loop "factor 1";
        Real c2_charge_loop "factor 2";
        Real c3_charge_loop "factor 3";
        //actual
  */
  //reduced
  Real n_CO_red_charge_loop(start = 1) "reduced speed";
  /*
  ...
  */
  ...
  //STATE 3a
  WorkingFluid.ThermodynamicState state_3_a_charge(p(start = 456931), T(start = from_degC(281.29))) "thermodynamic state of HEX outlet";
  ...
  //STATE 3a LOOP
  WorkingFluid.ThermodynamicState state_3_a_charge_loop(p(start = 456931), T(start = from_degC(281.29))) "thermodynamic state of HEX outlet";
  SI.Temperature T_3_a_charge_loop(start = from_degC(281.29)) "outlet temperature after HEX";
  SI.Pressure p_3_a_charge_loop(start = 456931) "Pressure after HEX";
  WorkingFluid.SpecificEnthalpy h_3_a_charge_loop(start = 685666) "HEX outlet enthalpy";
  WorkingFluid.SpecificEntropy s_3_a_charge_loop(start = 4079) "HEX outlet spec. entropy";
  //STATE 4 LOOP
  SI.Temperature T_4_charge_loop = from_degC(267.533) "state 4 temperature";
  //state 2 charge
...
//other turbine equations
  ...
  eta_is_TU_charge = (T_2_a_charge - T_1_charge)/(T_2_a_charge - T_1_is_charge);
/*
//-------------HEX 1 CHARGE//
//pressure loss
...
// Q_dot_HEXrej_charge = m_dot_WF_charge*(h_cold_out_HEX3_charge-state_1_charge.h);
*/
//T_2_charge=guess_T_2_charge(T_2_a_charge,p_2_a_charge, m_dot_WF_charge,T_amb );
//-------------STATES//
/*
//STATE 4 charge
...
/*
 ...
 h_hot_out_HEXrej_charge := WorkingFluid.specificEnthalpy(state_2_charge)-(Q_dot_HEXrej_charge/m_dot_WF_charge );    
...
/*
  ...
  delta_T_3_a_alg:=T_3_a_charge-540;
  i:=1;
  while delta_T_3_a_alg>delta_T_3_a_target loop 
  T_3_a_charge:=T_4_a_charge+250+i; 
  delta_T_3_a_alg:=T_3_a_charge-540;
  i:=i+1;
  end while;
  */
  i := 1;
//set
  T_3_a_charge_loop := 552;
  p_3_a_charge_loop := 452019;
  state_3_a_charge_loop := WorkingFluid.setState_pT(p_3_a_charge_loop, T_3_a_charge_loop);
  h_3_a_charge_loop := WorkingFluid.specificEnthalpy(state_3_a_charge_loop);
  s_3_a_charge_loop := WorkingFluid.specificEntropy(state_3_a_charge_loop);
// HEX 2 LOOP
// hot side
  cp_hot_HEX2_charge_loop := WorkingFluid.specificHeatCapacityCp(state_3_a_charge_loop);
//just inlet cp, not average cp
  C_hot_HEX2_charge_loop := m_dot_WF_charge*cp_hot_HEX2_charge_loop;
//cold side
  cp_cold_HEX2_charge_loop := WorkingFluid.specificHeatCapacityCp(state_4_a_charge);
//just inlet cp, not average cp
  C_cold_HEX2_charge_loop := m_dot_WF_charge*cp_cold_HEX2_charge_loop;
//variables for effectiveness
  C_min_HEX2_charge_loop := min(C_cold_HEX2_charge_loop, C_hot_HEX2_charge_loop);
  C_max_HEX2_charge_loop := max(C_cold_HEX2_charge_loop, C_hot_HEX2_charge_loop);
  C_r_HEX2_charge_loop := C_min_HEX2_charge_loop/C_max_HEX2_charge_loop;
  NTU_HEX2_charge_loop := UA_HEX2_charge/C_min_HEX2_charge_loop;
  eff_HEX2_charge_loop := (1 - exp(-NTU_HEX2_charge_loop*(1 - C_r_HEX2_charge_loop)))/(1 - C_r_HEX2_charge_loop*exp(-NTU_HEX2_charge_loop*(1 - C_r_HEX2_charge_loop)));
//heat transferred
  Q_dot_max_HEX2_charge_loop := C_min_HEX2_charge_loop*(T_3_a_charge_loop - T_4_a_charge);
  Q_dot_HEX2_charge_loop := eff_HEX2_charge_loop*Q_dot_max_HEX2_charge_loop;
//energy balance and outlet states
//cold side energy balance
  h_cold_out_HEX2_charge_loop := (Q_dot_HEX2_charge_loop/m_dot_WF_charge) + state_4_a_charge.h;
  outlet_coldside_HEX2_charge_loop := WorkingFluid.setState_ph(p_4_charge, h_cold_out_HEX2_charge_loop);
  T_4_charge_loop := outlet_coldside_HEX2_charge_loop.T;
//COMPRESSOR CHARGE LOOP
//reduced values compressor
  n_CO_red_charge_loop := n_CO_charge/sqrt(T_4_charge_loop)/(n_CO_nom_charge/sqrt(T_4_nom_charge));
  while i < 2 loop
//fix number of loops
    i := i + 1;
    T_3_a_charge_loop := T_3_a_charge_loop + 1;
    state_3_a_charge_loop := WorkingFluid.setState_pT(p_3_a_charge_loop, T_3_a_charge_loop);
    h_3_a_charge_loop := WorkingFluid.specificEnthalpy(state_3_a_charge_loop);
    s_3_a_charge_loop := WorkingFluid.specificEntropy(state_3_a_charge_loop);
// hot side
...
//heat transferred
  ...
  Q_dot_max_HEX2_charge = C_min_HEX2_charge*(T_3_a_charge - T_4_a_charge);
...
//
  ...
  T_4_charge = outlet_coldside_HEX2_charge.T;
    //compressor
    //p_3_charge:=beta_CO_charge * p_4_charge;    
  //p_3_charge:=beta_CO_charge * p_4_charge;    
  */
equation
  state_3_a_charge = WorkingFluid.setState_pT(p_3_a_charge_loop, T_3_a_charge_loop);
  h_3_a_charge = WorkingFluid.specificEnthalpy(state_3_a_charge);
  s_3_a_charge = WorkingFluid.specificEntropy(state_3_a_charge);
  T_3_a_charge = state_3_a_charge.T;
  p_3_a_charge = state_3_a_charge.p;
  cp_cold_HEX2_charge = WorkingFluid.specificHeatCapacityCp(state_3_a_charge);
/*

// hot side
  cp_hot_HEX2_charge = WorkingFluid.specificHeatCapacityCp(state_4_a_charge);
//just inlet cp, not average cp
...
end StaticMalta_open_loop_add_comp;
