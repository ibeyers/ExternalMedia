...
model TwoTank_plus_HEX_Malta_turb_comp_recup
  //--------------------------IMPORTS-----------------------------//
  ...
  //-------------HEX 1//
  //design
  parameter Real UA_HEX1_nom = 25443163;
  SI.MassFlowRate m_dot_solsalt_HEX1_nom = 540.269;
  //actual
  Real UA_HEX1;
  //outlet guess states
  HotTESLiquid.ThermodynamicState outlet_hotside_guess_HEX1(T(start = 270)) "Medium properties of HEX port at interface to tank 2";
  WorkingFluid.ThermodynamicState outlet_coldside_guess_HEX1(T(start = 570)) "Medium properties of HEX port at interface to tank 2";
  ...
  //-------------HEX RECUP//
  parameter Real UA_HEX_recup = 13279998;
  ...
  //actual
   ...
   WorkingFluid.ThermodynamicState outlet_coldside_guess_HEX2(T(start = 270)) "Medium properties of HEX port at interface to tank 2";   
   ...
   WorkingFluid.ThermodynamicState outlet_hotside_guess_HEX2(T(start = 120)) "Medium properties of HEX port at interface to tank 2";   
  ...
  //  SI.SpecificEnthalpy h_cold_out_HEX2;
   // WorkingFluid.ThermodynamicState outlet_hotside_HEX2(T(start = 120)) "Medium properties of HEX port at interface to tank 2";
   // WorkingFluid.ThermodynamicState outlet_coldside_HEX2(T(start = 270)) "Medium properties of HEX port at interface to tank 2";     
  ...
  /*
      ...
      //-------------HEX COLD//
      parameter Real UA_HEX_Cold = 15076653;
  ...
  SI.Pressure p_1(start = p_1_initial) " pressure at compressor inlet";
  // INITIAL
 ...
 parameter SI.Pressure  p_4_a_initial = 103276;
  ...
  //  SI.Temperature T_3_a_new(start= from_degC(260)) "outlet temperature after recuperation";
  ...
  WorkingFluid.ThermodynamicState state_4(p(start=p_4_initial), T(start=T_4_initial)) "thermodynamic state of turbine outlet";
  ...
  // SI.Temperature T_4_a_new "outlet temperature after recuperation";
      SI.Temperature T_4_a "outlet temperature after recuperation";  
  /*  
...
initial equation
  ...
  T_tank2 = T_start_tank2;
   // Q_dot_max_HEX2= C_min_HEX2*(T_4_initial - T_2);  //should acutally be T_4
  //state_4 = WorkingFluid.setState_pT(p_4_initial, T_4_initial);  
 // T_3_a=from_degC(263.38);
/*
...
//off-design
  ...
  outlet_hotside_guess_HEX1 = HotTESLiquid.setState_pT(p_amb, T_3_a);//NaK
...
//balanced operation
  m_dot_solsalt_HEX1 = C_hot_HEX1/cp_hot_ave_HEX1;
//mass balance
  m_out_tank1 + m_dot_solsalt_HEX1 = 0;
  m_dot_solsalt_HEX1 = m_in_tank2;
...
//To-Do unneccessary state
//-------------HEX RECUP//
  C_4_HEX_recup=m_dot_WF*WorkingFluid.specificHeatCapacityCp(state_4); //hot
  C_2_HEX_recup=m_dot_WF*WorkingFluid.specificHeatCapacityCp(state_4);  //cold
...
//-------------HEX2 (Recuperation)//  
//cold side
  outlet_coldside_guess_HEX2 = WorkingFluid.setState_pT(p_3_a,T_3_a_initial );
  ...
  // hot side
  outlet_hotside_guess_HEX2 = WorkingFluid.setState_pT(p_4_a_initial,T_4_a_initial);
  cp_hot_ave_HEX2=(WorkingFluid.specificHeatCapacityCp(state_4_initial) + WorkingFluid.specificHeatCapacityCp(outlet_hotside_guess_HEX2))/2; //should acutally be state_4
  ...
  eff_HEX2 =(C_hot_HEX2/C_min_HEX2)* (T_4 - T_4_a_new)/(T_4 - T_2); //calc T_4_a
  C_min_HEX2/C_max_HEX2=(T_3_a- T_2)/(T_4 - T_4_a_new); //calc T_3_a
  
   //heat transferred
//   Q_dot_HEX2=eff_HEX2*Q_dot_max_HEX2;
 //  Q_dot_max_HEX2= C_min_HEX2*(T_4_initial - T_2);  //should acutally be T_4
  //  Q_dot_max_HEX2= C_min_HEX2*(T_4 - T_2);  //should acutally be T_4
  //energy balance and outlet states
//calc T_tank2 from hot side energy balance
 // Q_dot_HEX2 = m_dot_WF*(state_4.h - h_hot_out_HEX2);
//calc T_tank3 from cold side energy balance
  //Q_dot_HEX2 = m_dot_WF*(h_cold_out_HEX2 - state_2.h);
//set outlets
 // outlet_hotside_HEX2 =  WorkingFluid.setState_ph(p_4_a, h_hot_out_HEX2);//NaK
//  outlet_coldside_HEX2 = WorkingFluid.setState_ph(p_3_a, h_cold_out_HEX2);//WF
T_3_a_new =from_degC(263.38);
...
/*
  ...
  C_4_HEX_recup=m_dot_WF*state_4.cp; //hot
  C_2_HEX_recup=m_dot_WF*state_2.cp;  //cold
  C_min_HEX_recup=min(C_4_HEX_recup,C_2_HEX_recup);
  C_max_HEX_recup=max(C_4_HEX_recup,C_2_HEX_recup);
  NTU_HEX_recup=UA_HEX_recup/C_min_HEX_recup;
  eff_HEX_recup = NTU_HEX_recup/(1 + NTU_HEX_recup);  
  ...
  T_1_a=T_4_a-89;
//-------------HEX COLD//
  C_T_1_a_HEX_Cold =m_dot_WF*state_1_a.cp;
...
//fixed pressure point
  T_1 = T_1_initial; //THIS NEEDS TO BE ADJUSTED LATER T-FIXPOINT AFTER REJEC
...
//--------------------------COMPONENT MANAGEMENT--------------------------//
  ...
  */
end TwoTank_plus_HEX_Malta_turb_comp_recup;
