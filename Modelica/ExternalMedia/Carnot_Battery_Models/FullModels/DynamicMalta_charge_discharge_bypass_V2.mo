...
model DynamicMalta_charge_discharge_bypass_V2
  //--------------------------IMPORTS-----------------------------//
  ...
  */
  ...
  SI.Energy int_energy_hot_tanks(displayUnit = "MWh");
  //-------------Charge//
  //design
  ...
  parameter SI.Power P_set_charge(displayUnit = "MW") = 180*1000*1000;
  ...
  SI.MassFlowRate m_dot_bypass_charge_exp;
  parameter SI.MassFlowRate m_dot_bypass_charge_exps_max = m_dot_WF_nom_charge*0.2;
  SI.MassFlowRate m_dot_WF_charge_exp(start = 766);
  SI.MassFlowRate m_dot_bypass_charge_exp_TEST;  
  Real cycle_mass_proxy_charge(start = 1380);
  //parameter Real cycle_mass_proxy_charge_set = 1407;
  parameter Real cycle_mass_proxy_charge_set = 1382.8;
  //-------------Discharge//
  //design
  ...
  parameter SI.Power P_set(displayUnit = "MW") = -52*1000*1000;
  ...
  parameter Real hot_to_cold_mass_flow_ratio = 2.04307;
  Real COP_system;
  SI.Energy E_total_loss_irr(displayUnit = "MWh", start = 0, fixed = true);
  //parameter SI.MassFlowRate m_dot_bypass = 50;
  SI.MassFlowRate m_dot_bypass;
  parameter SI.MassFlowRate m_dot_bypass_max = m_dot_WF_nom*0.25;
  SI.MassFlowRate m_dot_WF_exp(start = 762);
  Real cycle_mass_proxy(start = 1629);
  parameter Real cycle_mass_proxy_set = 1630.3;
  ...
  //--------------------------TANK 2
  //nominal
  parameter SI.Temperature T_tank2_nom = from_degC(279);
  ...
  //state 1 MIX charge
  SI.Pressure p_1_charge_mix(start = 104896) " pressure ";
  SI.Temperature T_1_charge_mix(start = from_degC(-66)) " temperature";
  WorkingFluid.ThermodynamicState state_1_charge_mix(p(start = 102161), T(start = from_degC(-66))) "thermodynamic state";
  WorkingFluid.SpecificEnthalpy h_1_charge_mix(start = 336932) "spec enthalpy";
  WorkingFluid.SpecificEntropy s_1_charge_mix(start = 3510) " spec. entropy";
  //state 4a charge
  ...
  SI.Temperature T_4_charge_guess(start = from_degC(274)) "state 4 temperature";
  ...
  //state 4 charge
  SI.Temperature T_4_charge(start = from_degC(274)) "state 4 temperature";
  ...
  SI.Temperature T_1_nom = from_degC(-52) "state 1 temperature";
  ...
  parameter SI.Temperature T_3_nom = from_degC(556) "design turbine inlet temperature";
  ...
  SI.Power P_mech_shaft_abs(displayUnit = "MW");
  ...
  SI.Temperature T_4(start = from_degC(279)) "outlet temperature of turbine";
  ...
  //STATE 4 MIX discharge
  SI.Pressure p_4_mix(start = 105039) " pressure at turb outlet";
  WorkingFluid.SpecificEnthalpy h_4_mix(start = 683188) "turbine outlet enthalpy";
  SI.Temperature T_4_mix(start = from_degC(270)) "outlet temperature of turbine";
  WorkingFluid.ThermodynamicState state_4_mix(p(start = 105039), T(start = from_degC(270))) "thermodynamic state of turbine outlet";
  WorkingFluid.SpecificEntropy s_4_mix "turbine outlet spec. entropy";
  //STATE 4 guess discharge
  SI.Temperature T_4_guess(start = from_degC(279)) "outlet temperature of turbine";
  ...
  SI.Temperature T_4_a(start = from_degC(100)) "outlet temperature after recuperation";
  ...
  Modelica.Units.SI.ReactivePower Q_GC(start = -50*1000*1000, displayUnit = "Mvar");
  //helper variable
  Modelica.Units.SI.ApparentPower S_GC_set;
  ...
  //------------------SYNCHRONOUS MACHINE
  //parameters
  parameter SI.ApparentPower S_SM_nom(displayUnit = "MVA") = 213*1000*1000;
  ...
  SI.Power P_mech_RO_abs(displayUnit = "MW");
  //losses
  ...
  //charge PID
/*
  Modelica.Blocks.Continuous.LimPID PID_bypass_charge(k = 0.000001, Ti = 5, Td = 0.1, yMax = 766, yMin = 0, wp = 0.9, wd = 0.2, initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = 0.01) annotation(
    Placement(transformation(origin = {36, 32}, extent = {{-10, -10}, {10, 10}})));

  Modelica.Blocks.Continuous.LimPID PID_low_press_charge(yMax = 500000, yMin = 20000, initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = 100000, k = 3, Ti = 1, wp = 0.9, wd = 0.2, Td = 0.01) annotation(
    Placement(transformation(origin = {-50, 66}, extent = {{-10, -10}, {10, 10}})));
 */

    //discharge PID

    Modelica.Blocks.Continuous.LimPID PID_bypass(initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = 1, yMax = 762, yMin = 0, k = -0.00001, wp = 0.9, wd = 0.2, Ti = 1, Td = 0.1) annotation(
      Placement(transformation(origin = {-50, 28}, extent = {{-10, -10}, {10, 10}})));
   // works k = 0.0001, wp = 0.9, wd = 0.2, Ti = 10, Td = 0.1

    Modelica.Blocks.Continuous.LimPID PID_low_press(yMax = 500000, yMin = 20000, initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = 100000, k = 10, Ti = 1, wp = 0.9, wd = 0.2, Td = 0.01) annotation(
      Placement(transformation(origin = {-50, 66}, extent = {{-10, -10}, {10, 10}})));


/*
      Modelica.Blocks.Sources.Ramp ramp(height = 100, duration = 900, offset = 0, startTime = 0) annotation(
...
//control loops
//charge
  T_4_charge_guess = from_degC(274);
  T_4_a_charge_guess = from_degC(18.14);
//discharge
  T_4_guess = from_degC(279);
...
//PID charge
/*
  P_mech_RO = PID_bypass_charge.u_s;
  P_mech_shaft_charge = PID_bypass_charge.u_m;
  m_dot_bypass_charge_exp = PID_bypass_charge.y;
*/
  m_dot_bypass_charge_exp_TEST=0;
  //m_dot_bypass_charge_exp = ramp.y;
//When charge PID is turned off, this must be uncommented
 m_dot_bypass_charge_exp = 0;
/*
  cycle_mass_proxy_charge_set = PID_low_press_charge.u_s;
  cycle_mass_proxy_charge = PID_low_press_charge.u_m;
...
//PID discharge
  ...
  P_mech_RO_abs = PID_bypass.u_s;
  P_mech_shaft_abs = PID_bypass.u_m;
  m_dot_bypass = PID_bypass.y;


//When discharge PID is turned off, this must be uncommented
  //m_dot_bypass = 0;

  cycle_mass_proxy_set = PID_low_press.u_s;
  cycle_mass_proxy = PID_low_press.u_m;
  p_1 = PID_low_press.y;
//When discharge PID is turned off, this must be uncommented
 // p_1 = 100000;
  ...
  P_elec = P_TR_HV;
  P_mech_RO_abs = abs(P_mech_RO);
  P_mech_shaft_abs = abs(P_mech_shaft);
//-------------SYSTEM TANKS//
  ...
  int_energy_hot_tanks = int_energy_tank1 + int_energy_tank2;
//-------------SYSTEM CHARGE//
  ...
  cycle_mass_proxy_charge = ((((p_1_charge_mix/(287.047*T_1_charge_mix)) + (p_4_a_charge/(287.047*T_4_a_charge)))/2)*UA_HEX3_nom/100000) + ((((p_4_a_charge/(287.047*T_4_a_charge)) + (p_4_charge/(287.047*T_4_charge)))/2)*UA_HEX2_nom/100000) + ((((p_2_charge/(287.047*T_2_charge)) + (p_3_a_charge/(287.047*T_3_a_charge)))/2)*UA_HEX2_nom/100000) + ((((p_3_charge/(287.047*T_3_charge)) + (p_3_a_charge/(287.047*T_3_a_charge)))/2)*UA_HEX1_nom/100000);
//-------------SYSTEM DISCHARGE//
  P_mech_shaft = P_mech_CO + P_mech_TU;
  Q_pump = -Q_dot_HEX1;
  eta_heat_to_power = P_mech_shaft/Q_dot_hightemp_res;
  eta_heat_to_power_sys = P_elec/Q_dot_hightemp_res;
  ...
  cycle_mass_proxy = ((((p_1/(287.047*T_1)) + (p_1_a/(287.047*T_1_a)))/2)*UA_HEX3_nom/100000) + ((((p_4_a/(287.047*T_4_a)) + (p_4_mix/(287.047*T_4_mix)))/2)*UA_HEX2_nom/100000) + ((((p_2/(287.047*T_2)) + (p_3_a/(287.047*T_3_a)))/2)*UA_HEX2_nom/100000) + ((((p_3/(287.047*T_3)) + (p_3_a/(287.047*T_3_a)))/2)*UA_HEX1_nom/100000);
//MODE 1 CHARGE
  if Mode == 1 then
    ...
    COP_system_charge = Q_dot_hightemp_res/P_elec_charge;
    COP_system = Q_dot_hightemp_res/P_elec;
//NaK
...
//MODE 2 DISCHARGE
  elseif Mode == 2 then
    ...
    COP_system = Q_dot_hightemp_res/P_elec;
//NaK
    outlet_coldside_HEX1_charge = HotTESLiquid.setState_ph(p_tank1_nom, solsalt_tank1_nom.h);
//NaK
...
//-------------EXPANDER CHARGE//
// mass flow rate
  m_dot_WF_charge_exp = m_dot_WF_charge - m_dot_bypass_charge_exp;
//speed
...
//reduced values expander
  ...
  G_TU_red_charge = m_dot_WF_charge_exp*sqrt(T_2_a_charge)/p_2_a_charge/(m_dot_WF_nom_charge*sqrt(T_2_a_nom_charge)/p_2_a_nom_charge);
  m_dot_WF_charge_exp/m_dot_WF_nom_charge = alpha_charge*sqrt(T_2_a_nom_charge/T_2_a_charge)*sqrt((beta_TU_charge^2 - 1)/(beta_TU_nom_charge^2 - 1));
...
//other turbine equations
  ...
  P_mech_TU_charge = m_dot_WF_charge_exp*(h_1_charge - h_2_a_charge);
  P_loss_irr_TU_charge = T0*m_dot_WF_charge_exp*(s_1_charge - s_2_a_charge);
  der(E_loss_irr_TU_charge) = P_loss_irr_TU_charge;
//-------------BYPASS EXPANDER MIX CHARGE//
  p_1_charge_mix = p_1_charge;
  m_dot_WF_charge*h_1_charge_mix = m_dot_WF_charge_exp*h_1_charge + m_dot_bypass_charge_exp*h_2_a_charge;
  state_1_charge_mix = WorkingFluid.setState_ph(p_1_charge_mix, h_1_charge_mix);
  T_1_charge_mix = state_1_charge_mix.T;
  s_1_charge_mix = WorkingFluid.specificEntropy(state_1_charge_mix);
//-------------HEX 1 CHARGE//
//pressure loss
...
//pressure loss
  ...
  p_1_charge_mix = p_4_a_charge + delta_P_HEX3_charge;
//bypass
//off-design
...
//charge: inlet cold side= state 1 MIX inlet hot side=tank 3
  outlet_hotside_guess_HEX3_charge = ColdTESLiquid.setState_pT(p_tank4_nom, T_1_charge_mix + 9);
//bypass
//
...
//
  cp_cold_ave_HEX3_charge = (WorkingFluid.specificHeatCapacityCp(state_1_charge_mix) + WorkingFluid.specificHeatCapacityCp(outlet_coldside_guess_HEX3_charge))/2;
//bypass
...
//this eff_HEX3_charge formulation is computationally expensive
  ...
  Q_dot_max_HEX3_charge = C_min_HEX3_charge*(T_tank3 - T_1_charge_mix);
//bypass
...
//calc T_4_a_charge from cold side energy balance
  Q_dot_HEX3_charge = m_dot_WF_charge*(h_cold_out_HEX3_charge - state_1_charge_mix.h);
//bypass
//set outlets
...
//irrev
  P_loss_irr_HEX3_charge = T0*((m_dot_WF_charge*(s_4_a_charge - s_1_charge_mix)) + (m_dot_methanol_HEX3_charge*(outlet_hotside_HEX3_charge.s - coldliq_tank3_state.s)));
//bypass
...
//-------------EXPANDER DISCHARGE//
// mass flow rate
  m_dot_WF_exp = m_dot_WF - m_dot_bypass;
//speed
  n_TU = n_CO;
//parameters
...
//reduced values turbine
  ...
  G_TU_red = m_dot_WF_exp*sqrt(T_3_guess)/p_3/(m_dot_WF_nom*sqrt(T_3_nom)/p_3_nom);
  m_dot_WF_exp/m_dot_WF_nom = alpha*sqrt(T_3_nom/T_3_guess)*sqrt((beta_TU^2 - 1)/(beta_TU_nom^2 - 1));
  eta_is_TU_red = (1 - t*(1 - n_TU_red)^2)*(n_TU_red/G_TU_red)*(2 - ((n_TU_red/G_TU_red)));
//other turbine equations
  beta_TU = p_3/p_4;
  eta_is_TU = (h_3 - h_4)/(h_3 - h_4_is);
  P_mech_TU = m_dot_WF_exp*(h_4 - h_3);
  P_loss_irr_TU = T0*m_dot_WF_exp*(s_4 - s_3);
  der(E_loss_irr_TU) = P_loss_irr_TU;
//-------------BYPASS MIX DISCHARGE//
  p_4_mix = p_4;
  m_dot_WF*h_4_mix = m_dot_WF_exp*h_4 + m_dot_bypass*h_3;
  state_4_mix = WorkingFluid.setState_ph(p_4_mix, h_4_mix);
  T_4_mix = state_4_mix.T;
  s_4_mix = WorkingFluid.specificEntropy(state_4_mix);
//-------------HEX 1 DISCHARGE//
//pressure loss
...
//MODE 1 CHARGE
  if Mode == 1 then
    outlet_hotside_HEX1 = HotTESLiquid.setState_ph(p_tank2_nom, solsalt_tank2_nom.h);
  else
    outlet_hotside_HEX1 = HotTESLiquid.setState_ph(p_tank2_nom, h_hot_out_HEX1);
  end if;
//NaK
...
//irrev
  P_loss_irr_HEX2 = T0*((m_dot_WF*(s_4_a - s_4_mix)) + (m_dot_WF*(s_3_a - s_2)));
//bypass
...
//state 4 guess DISCHARGE
  T_4_mix = T4_guess_control_discharge.u;
//bypass
  T_4_guess = T4_guess_control_discharge.y;
  state_4_guess = WorkingFluid.setState_pT(p_4_mix, T_4_guess);
//bypass
...
//tanks
    ...
    if T_3_charge > 873.150000 - 1 then
      terminate("Temperature after compressor too high");
    end if;
...
//turbomachinery
    ...
    if m_dot_bypass > m_dot_bypass_max then
      terminate("maximum bypass flow reached");
    end if;
...
end DynamicMalta_charge_discharge_bypass_V2;
