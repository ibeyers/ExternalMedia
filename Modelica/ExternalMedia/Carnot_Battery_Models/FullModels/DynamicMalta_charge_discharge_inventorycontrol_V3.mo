...
model DynamicMalta_charge_discharge_inventorycontrol_V3
  //--------------------------IMPORTS-----------------------------//
  ...
  Real cycle_mass_proxy(start = 1629); 
  parameter Real cycle_mass_proxy_nom=1630.3;  
...
//turbomachinery
    ...
    if cycle_mass_ratio < 0.45 then
      terminate("cycle mass too low");
    end if;
    if cycle_mass_ratio > 1.1 then
